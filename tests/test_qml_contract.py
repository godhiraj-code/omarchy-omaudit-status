...
class QmlContractTests(unittest.TestCase):
    ...
    def test_manifest_declares_singleton_service_and_bar_widget(self):
        ...
        self.assertFalse(self.manifest["keepLoaded"])
    ...
    def test_service_uses_fixed_paths_arguments_and_bounded_streaming(self):
        ...
        self.assertIn('function replaceUnstartedProcess()', self.service)
        self.assertIn('scanProcessComponent.createObject(root)', self.service)
        self.assertIn('retired.destroy()', self.service)
        self.assertIn('onRead: function(chunk) { root.ingestStdout(chunk) }', self.service)
        self.assertIn('if (scanning || (scanProcess && scanProcess.running))', self.service)
        self.assertIn('property int _configurationGeneration: 0', self.service)
        self.assertIn('property int _activeGeneration: -1', self.service)
        self.assertIn('property bool _refreshPending: false', self.service)
        self.assertIn('var scopeChanged = includeBuiltins !== configuredBuiltins', self.service)
        self.assertIn('StatusModel.shouldPublishScan(root._activeGeneration,', self.service)
    ...
    def test_panel_only_reads_the_shared_service_and_exposes_required_actions(self):
        ...
        self.assertNotIn('bar.barConfig', self.panel)
        self.assertNotIn('onHostBarConfigChanged', self.panel)
        ...
        self.assertIn('id: externalActionDelay', self.panel)
        self.assertIn('root.close()', self.panel)
        self.assertIn('pendingExternalAction = action', self.panel)
        self.assertIn('property bool popoutSwitchClosing: false', self.panel)
        self.assertIn('function closeForPopoutSwitch()', self.panel)
        self.assertIn('bar.switchPanelFrom(root, direction)', self.panel)
